...
**현장의 문제를 정의하고 → 기술 대안을 데이터로 검증하고 → 의사결정으로 완결합니다.**

일반적인 IT 기획자가 기술을 "발주 대상"으로 다룬다면, 저는 직접 모델을 만들고 비교·검증해 본 경험을 바탕으로 **도입 타당성·비용·성능을 스스로 판단**합니다. 이것이 개발 부서·외부 파트너와 가장 정확하게 소통하고, IT 예산과 일정을 현실적으로 통제할 수 있는 근거입니다.

| | 강점 | 증거 |
| :---: | :--- | :--- |
| **①** | **현장 기반 문제 정의** — 공공 플랫폼 운영 2년간 데이터로 문제를 정의하고 개발 우선순위를 결정 | 코리아경기도(배달특급) 기획·운영 2년 |
| **②** | **데이터 기반 기술 검증** — 기술을 그냥 도입하지 않고 비용·성능을 비교(BMT)해 선정 | RAG 12개 조합 비교·선정 · faithfulness 근거로 LoRA 제외 판단 |
| **③** | **End-to-End 완결** — 기획·발주·계약부터 모델 설계·배포까지 직접 | 나라장터 입찰 단독 수행 · AI 서비스 배포 |

---

## 💼 Career — 공공 플랫폼 기획·운영

### 코리아경기도주식회사 | 플랫폼사업실 주임 | 2023.12 ~ 2025.12 (2년)

공공배달앱 '배달특급'의 데이터 분석·예산 관리·약관 거버넌스·운영 용역 발주 담당.

- **계약·사업관리** — 가맹점 관리 용역 나라장터 경쟁입찰 단독 수행 (RFP 작성 → 평가위원회 운영 → 계약 체결 전 과정). 개발사·운영사·발주처·외주사 구조에서 예산 집행·대관 관리
- **투자 우선순위 결정** — 누적 CS 데이터 분석으로 백오피스 UI 개선의 필요성을 수치 입증, 만년 후순위 안건을 개발 1순위로 승인받아 배포
- **다자 조율·프로세스 설계** — G드림카드 환불 규정 한계를 편의점 본사·결제사·고객센터 3사 조율로 해결, 전산 정산 프로세스 재설계
- **규제 대응** — 개인정보보호법·위치정보법·전자금융거래법 분석으로 정책 문서 7종 전면 개정
- **데이터 운영** — 일 수천 건 주문 데이터로 핵심 KPI 도출·리포트 체계 구축, 부서 분산 데이터 일원화 대시보드

> 기획자로서 한정된 IT 자원의 투자 우선순위를 데이터로 결정하고, 복잡한 외부 파트너 구조에서 계약·프로세스를 설계한 경험.

---

## 🤖 Hands-on AI Projects — 기술 통제력의 증거

> *아래는 AI 엔지니어링 부트캠프(2025.12~) 및 개인 프로젝트입니다. IT 기획자로서 "기술을 직접 검증·통제할 수 있다"는 점을 보여주는 포트폴리오입니다.*

<table>
  <thead>
    <tr><th width="20%">프로젝트</th><th width="56%">설명</th><th width="24%">기술 스택</th></tr>
  </thead>
  <tbody>
    <tr>
      ...
      <td>
        ...
        • 임베딩 3종 × LLM 4종 = <b>12개 조합 BMT</b> 후 비용·성능 기준으로 KURE-v1 + Phi-4-mini 선정<br>
        • 평가 파이프라인 구축 → <b>Hit@5 0.91 / MRR 0.85 / nDCG 0.81</b><br>
        • <i>(공공조달 도메인 지식을 직접 로직화 — 입찰 실무 경험을 기술로 전환)</i>
      </td>
      <td><code>KURE-v1</code> <code>ChromaDB</code> <code>BM25(Kiwi)</code> <code>bge-reranker-v2-m3</code> <code>Phi-4-mini</code> <code>GCP</code></td>
    </tr>
    <tr>
      <td><b><a href="https://github.com/EuijeongHan/forg">foRG</a></b><br><sub>📈 금융/공시</sub></td>
      <td>
        <b>DART 공시 자동 요약·알림 서비스</b> · <i>금융 도메인 AI 적용</i><br>
    ...
    <tr>
      <td><b>카페 광고 AI</b><br><sub>🏪 소상공인 · <i>2026.07</i></sub></td>
      <td>
        <b>소상공인 광고·콘텐츠 생성 AI</b><br>
        • 카피(LLM) + 브랜드 톤(RAG) + 브랜드 비주얼(Diffusion·LoRA)<br>
        • 카페 운영 경험을 기반으로 한 도메인 특화 설계
      </td>
...
| 프로젝트 | 설명 | 스택 |
| :--- | :--- | :--- |
| [알약 탐지 (HealthEat)](https://github.com/EuijeongHan/pill_detection_project) | 알약 객체 탐지·분류(73종) · Data Engineering 리드 · Kaggle mAP@0.5 0.986 | YOLO11 · PyTorch · DETR |
| [foRG-mini](https://github.com/EuijeongHan/streamlit_forg-mini) | DART 공시 조회 경량 데모 | Streamlit · Phi-4-mini |
| [Movie Sentiment](https://github.com/EuijeongHan/codeit_mission18-movie-review) | 영화 리뷰 감성분석 풀스택 앱 | KcELECTRA · FastAPI |
| [MNIST & PillFinder](https://github.com/EuijeongHan/codeit_mission17-pill-mnist) | 실시간 숫자 인식 + 알약 탐지 · Docker 배포 | PyTorch · YOLO11 · Docker |
...
**Data & Analysis** Python · Pandas · NumPy · SQL · Excel(Pivot/Power Query)
**AI & ML** PyTorch · scikit-learn · Hugging Face · LangChain
**LLM & RAG** ChromaDB · KURE-v1 · Claude API · BM25(Kiwi)
**Serving & Infra** FastAPI · Docker · PostgreSQL · GCP · Git

---

## 👋 About Me

깊이 파고드는 힘, 데이터로 판단하는 눈, 방법을 찾아 뚫고 나가는 돌파력 — 분야를 바꿔가며 단련해온 무기입니다. 카페 운영, 공공 배달 플랫폼 기획·운영을 거쳐 AI 엔지니어링까지, 매번 새 도메인을 파고들었습니다. IT 기획자로서 저의 차별점은 **기술을 발주만 하는 게 아니라 직접 검증하고 통제할 수 있다**는 것입니다.

### ⏳ Timeline

```
2025.12 - Present   Codeit AI Engineer Bootcamp (8기)
2023.12 - 2025.12   (주)코리아경기도 플랫폼사업실 (배달특급 기획/운영, 2년)
