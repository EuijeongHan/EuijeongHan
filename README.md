...
| | 강점 | 증거 |
| :---: | :--- | :--- |
| **①** | **낯선 도메인을 빠르게 판다** — 처음 보는 분야도 구조부터 파악해 풀어야 할 문제로 정의합니다. | 공공조달·금융·소상공인, 서로 다른 3개 도메인을 직접 서비스로 |
...
산업공학에서 출발해 재경직 준비, 카페 운영, 공공 배달 플랫폼 기획·운영을 거쳐 AI 엔지니어가 됐습니다. 한 분야에 머문 적이 없고, 매번 낯선 곳에 들어가 적응해온 과정이 지금의 저를 만들었습니다.

배달특급(경기도 공공배달 플랫폼) 시절엔 유일한 공학 배경 인력으로 데이터 추출·보안 문서화·나라장터 조달 프로세스를 담당했습니다. 그때 부딪힌 현장의 문제들이 지금 만드는 AI 서비스의 출발점입니다 — 입찰메이트(공공조달)도, foRG(금융·공시)도, 7월에 출시할 카페 광고 AI(소상공인)도, 모두 제가 직접 겪은 도메인에서 나왔습니다.
